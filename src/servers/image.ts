...
import { upload, UploadResponse } from "../services/qiniu.js";
...
server.addTool({
  ...
  execute: async (args) => {
    ...
    const allImages: {
      png: { input: string; output: string }[];
      webp: { input: string; output: string }[];
      other: { input: string; output: string }[];
    } = {
      png: [],
      webp: [],
      other: [],
    ...
    const pngCompressPs: Promise<string | OutputInfo>[] = [];
    const webpCompressPs: Promise<OutputInfo>[] = [];
    const otherCompressPs: Promise<OutputInfo>[] = [];
    ...
    uploadResponses.forEach((uploadResponse) => {
      const originalIdx = flatImages.findIndex(
        ({ output: opt }) => opt === uploadResponse.data!.originalUrl
      );
      ...
      if (uploadResponse.code === 200) {
        ...
        const idx = failedUploads.findIndex(
          ({ input }) => input === uploadResponse.data!.originalUrl
        );
        if (idx < 0) {
          failedUploads.push({
            original,
    ...
    succeedUploads.forEach((item) => {
      ...
      resText += `压缩后图片地址：${item.output}，原图大小：${formatBytes(
        originSize
      )}，压缩后大小：${formatBytes(newSize)}，压缩比例：${saveRate.toFixed(
        2
      )}%\n`;
    });
